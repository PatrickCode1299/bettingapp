...
<div class="container fund-account-container">
    ...
    <ul class="list-group">
        <?php 
         ...
         $fetchwinnings="SELECT DISTINCT winningstatus,gamerefid,gamedate,payment_request FROM usersgameholder WHERE gameowner='$onlineuser' AND winningstatus=1";
         ...
         while($getnewinfo=mysqli_fetch_assoc($querysql)){
            if($getnewinfo['winningstatus']==1 && $getnewinfo['payment_request']=='yes'){
               ...
               echo "<li class='list-group-item'>Ticket id:".$getnewinfo['gamerefid']." Won  <button class='btn btn-primary btn-md' onclick=setpaymentreq('".$gamerefid."') disabled='true'>You already requested payment </button></li>";
            }
